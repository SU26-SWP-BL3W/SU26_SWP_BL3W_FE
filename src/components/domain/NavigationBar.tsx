...
export function NavigationBar() {
      ...
      <aside className="w-full md:w-64 bg-[var(--bg-panel)] border-b md:border-b-0 md:border-r border-[#2dd4bf]/30 flex flex-col justify-between p-5 shrink-0 z-50 md:fixed md:left-0 md:top-0 md:bottom-0">
        <div className="flex flex-col gap-6">
          ...
          <div className="flex flex-col gap-3 pb-4 border-b border-[var(--border-muted)]">
            <Link href="/" className="font-display font-bold text-lg text-[#2dd4bf] tracking-widest uppercase flex items-center gap-2.5">
          ...
          <div className="p-3 bg-[var(--bg-input)] border border-[#2dd4bf]/40 hud-clipped flex flex-col gap-1">
            ...
            <span className="font-display text-xs font-bold text-[var(--text-primary)] truncate">
              {user?.FullName || "TS. Nguyễn Văn Cố Vấn"}
            </span>
            <span className="font-mono text-[10px] text-[var(--text-muted)]">
              Chuyên môn: AI & Web Architecture
            </span>
          </div>
  ...
  // ─────────────────────────────────────────────────────────────
  // CHẾ ĐỘ 1B: NAVBAR DỌC (FIXED LEFT SIDEBAR DOCK) - KHI THÍ SINH VÀO SỰ KIỆN
  // ─────────────────────────────────────────────────────────────
  if (isEventWorkspace) {
    return (
      ...
      <div className="flex items-center gap-6 md:gap-8">
        ...
        <div className="hidden md:flex gap-5 items-center font-mono text-xs">
          ...
          {user && (
            ...
            >
              <span>{isMentor ? "Vào Mentor Panel" : "Vào sự kiện đang thi"}</span>
        ...
        {user ? (
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="hidden lg:flex items-center gap-1.5 border border-[var(--border-muted)] px-2 py-1 bg-[var(--bg-input)]">
              ...
              <button onClick={() => login("TeamLeader")} className="text-[10px] font-mono text-[var(--accent-team)] font-bold hover:underline" title="Đội Trưởng">Leader</button>
              <span className="text-[10px] text-[var(--border-muted)]">|</span>
              <button onClick={() => login("TeamMember")} className="text-[10px] font-mono text-[var(--accent-team)] hover:underline" title="Thành Viên">Member</button>
              <span className="text-[10px] text-[var(--border-muted)]">|</span>
              <button onClick={() => login("Mentor")} className="text-[10px] font-mono text-[#2dd4bf] font-bold hover:underline" title="Cố Vấn">Mentor</button>
              <span className="text-[10px] text-[var(--border-muted)]">|</span>
              <button onClick={() => login("Judge")} className="text-[10px] font-mono text-[var(--accent-judge)] hover:underline" title="Giám Khảo">Judge</button>
              <span className="text-[10px] text-[var(--border-muted)]">|</span>
              <button onClick={() => login("Coordinator")} className="text-[10px] font-mono text-[var(--accent-coordinator)] hover:underline" title="Ban Tổ Chức">Coord</button>
            </div>
