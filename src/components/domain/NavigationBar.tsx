...
export function NavigationBar() {
  ...
  // ─────────────────────────────────────────────────────────────
  // CHẾ ĐỘ 1B: NAVBAR DỌC - KHI THÍ SINH VÀO SỰ KIỆN / WORKSPACE
  // ─────────────────────────────────────────────────────────────
  if (isEventWorkspace) {
    return (
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="p-3 bg-[var(--bg-input)] border border-[var(--border-muted)] hud-clipped flex flex-col gap-1">
            <span className={`font-mono text-[9px] font-bold uppercase tracking-widest ${roleName === "Guest" ? "text-[var(--text-muted)]" : "text-[var(--accent-primary)]"}`}>
              {roleName === "Guest" ? "KHÁCH VÃNG LAI" : "ĐANG THI ĐẤU"}
            </span>
            <span className="font-display text-xs font-bold text-[var(--text-primary)] truncate">
            ...
            <span className={`font-mono text-[10px] font-bold ${roleName === "Guest" ? "text-[var(--text-muted)] opacity-70" : "text-[var(--accent-team)]"}`}>
              {roleName === "Guest" ? "Chưa tham gia Đội thi" : `Đội: ${team?.name || "Cyber_Knights"}`}
            </span>
          </div>
          ...
          <nav className="flex flex-col gap-1.5 font-mono text-xs">
            ...
            >
              <span>{roleName === "Guest" ? "➕" : "👥"}</span> {roleName === "Guest" ? "Tạo hoặc Tìm Đội Thi" : "Quản Lý Đội Thi"}
            </Link>

            {/* Chỉ hiển thị Bài Nộp & Phúc Khảo cho Thí Sinh Đội Thi (TeamLeader / TeamMember) */}
            {roleName !== "Guest" && (
              <>
                <Link
                  href="/my-submissions"
    ...
    <nav className="w-full h-16 border-b border-[var(--border-muted)] bg-[var(--bg-panel)] flex items-center justify-between px-6 shrink-0 z-30 shadow-sm">
      ...
      <div className="flex items-center gap-6 md:gap-8">
        ...
        <div className="hidden md:flex gap-5 items-center font-mono text-xs">
          ...
          {user && (
            <Link
              href={isMentor ? "/mentor/tracks" : `/events/${currentEventId}`}
            ...
            >
              <span>{isMentor ? "Vào Bàn Cố Vấn" : "Vào sự kiện đang thi"}</span>
      ...
      <div className="flex items-center gap-3">
        ...
        {user ? (
          <div className="flex items-center gap-3">
            <span className={`font-mono text-xs font-bold ${isMentor ? "text-[#2dd4bf]" : "text-[var(--accent-team)]"}`}>
              {roleName}
            </span>
            <button
              onClick={logout}
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="hidden lg:flex items-center gap-1.5 border border-[var(--border-muted)] px-2 py-1 bg-[var(--bg-input)] font-mono">
              <button onClick={() => login("TeamLeader")} className="text-[10px] text-[var(--accent-team)] font-bold hover:underline" title="Đội Trưởng">Leader</button>
              <span className="text-[10px] text-[var(--border-muted)]">|</span>
              <button onClick={() => login("TeamMember")} className="text-[10px] text-[var(--accent-team)] hover:underline" title="Thành Viên">Member</button>
              <span className="text-[10px] text-[var(--border-muted)]">|</span>
              <button onClick={() => login("Mentor")} className="text-[10px] text-[#2dd4bf] font-bold hover:underline" title="Cố Vấn">Mentor</button>
              <span className="text-[10px] text-[var(--border-muted)]">|</span>
              <button onClick={() => login("Judge")} className="text-[10px] text-[var(--accent-judge)] hover:underline" title="Giám Khảo">Judge</button>
              <span className="text-[10px] text-[var(--border-muted)]">|</span>
              <button onClick={() => login("Coordinator")} className="text-[10px] text-[var(--accent-coordinator)] hover:underline" title="Ban Tổ Chức">Coord</button>
            </div>
