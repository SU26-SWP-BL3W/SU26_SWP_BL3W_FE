    ...
    <main className="hud-lattice flex flex-1 flex-col">
      ...
      <div className="mx-auto w-full max-w-[var(--container-max)] px-6 py-6 flex gap-6 flex-1">
        ...
        <div className="flex-1 min-w-0 flex flex-col gap-4">
          ...
          {/* ── My Joined Event Banner (Always visible with smart fallback) ── */}
          <div className="p-5 bg-[var(--bg-panel)] border border-[var(--accent-team)]/40 hud-clipped flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-[0_0_20px_rgba(56,189,248,0.08)]">
            <div className="flex flex-col gap-1">
              <div className="flex items-center gap-2 font-mono text-[10px] font-bold text-[var(--accent-team)] uppercase tracking-widest">
                <span className="w-2 h-2 rounded-full bg-[var(--accent-team)] animate-pulse" />
                ⭐ SỰ KIỆN CỦA TÔI {team ? `(ĐỘI: ${team.name})` : ""}
              </div>
              <h2 className="font-display text-xl font-bold text-[var(--text-primary)]">
                {team ? team.eventName : "SEAL Hackathon 2026 — Đang Thi Đấu"}
              </h2>
              <div className="flex items-center gap-3 font-mono text-xs text-[var(--text-muted)] mt-0.5">
                <span>Trạng thái: <strong className="text-[var(--color-success)]">{team ? team.status.toUpperCase() : "ĐÃ ĐĂNG KÝ THAM GIA"}</strong></span>
                <span>·</span>
            ...
            <Link href={team ? `/events/${myEventId}` : "/login"}>
              <button className="hud-clipped px-5 py-2.5 bg-[var(--accent-team)] text-[var(--bg-base)] font-mono font-bold text-xs tracking-wider uppercase hover:bg-white transition-all shadow-sm shrink-0">
